...
namespace AbpMvc5.Tests
{
    public abstract class AbpMvc5TestBase : AbpIntegratedTestBase<AbpMvc5TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AbpMvc5TestBase()
        {
        ...
        protected void UsingDbContext(Action<AbpMvc5DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpMvc5DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpMvc5DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpMvc5DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpMvc5DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpMvc5DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpMvc5DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpMvc5DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpMvc5DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpMvc5DbContext, Task<T>> func)
        {
            T result;
